# November 30, 2025
# Code to run the Pima dataset experiment on AWS
################################################################################
# Load libraries
library(KernSmooth)
library(locfit)
...
cat("File lr/nb/sb comparison: Pima data\n")
cat("Using spline density ratio estimate\n")

################################################################################
# Load and preprocess data
# Users should change this to point to wherever the data are stored.
pima <- read_csv(here("Real Data/data/pima-indians-diabetes.csv"))

# Outcome variable
yval <- pima$X9

# Drop outcome from predictors
all.funcs <- na.omit(pima)
all.funcs$X9 <- NULL
...
m.seq <- seq(50, 768, by = 8)

# Define the numnber of iterations
num.i <- 200

# This is the main function to run the experiment
run_pima_function <- function(rep_idx) {
...
result_list <- Q(run_pima_function,
...
write.csv(summary_df,
          paste0("../output_data/aws-pima-summary-", num.i, ".csv"),
          row.names = FALSE)
saveRDS(summary_df,
        paste0("../output_data/aws-pima-summary-", num.i, ".RDS"))
